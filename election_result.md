...
![](election_result_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
election_by_region %>% 
    select(-`State Code`) %>% 
    mutate(prop_votes = round(state_votes / state_subtotal,2)) %>% 
    ggplot(aes(x = Region, y = prop_votes, fill = candidate )) +
    geom_bar(stat = "identity", position = position_dodge()) + 
...
![](election_result_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->
...
![](election_result_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
![](election_result_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->

``` r
ggplotly(election_result_map)
```
